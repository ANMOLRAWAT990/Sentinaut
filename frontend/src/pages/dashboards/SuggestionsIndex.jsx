...
export function SuggestionsIndex() {
  ...
  const fetchInsights = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8000/api/insights?property=${propName}`);
  ...
  const generateInsights = async () => {
    ...
    try {
      const propName = activeProperty || user?.property || 'Unassigned';
      const res = await fetch(`http://localhost:8000/api/insights/generate?property=${propName}`, {method: 'POST'});
      if(res.ok) {
