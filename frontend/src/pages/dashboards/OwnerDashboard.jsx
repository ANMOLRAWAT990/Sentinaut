  ...
  const handleInviteManager = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        addToast(`Invitation link sent to ${managerEmail}! Default password is 'password123'. They can register via the link sent to their email.`, 'success', 10000);
      } else {
        const data = await res.json();
            ...
            {['trial'].includes(currentPlan) ? (
              <div className="flex flex-col items-center justify-center py-8 px-4 text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 max-w-sm mt-1">Upgrade to the Resort plan to unlock AI-driven competitor analysis.</p>
                </div>
