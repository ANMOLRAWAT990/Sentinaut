...
export function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      login({ email: data.user.email, role: data.user.role, name: data.user.name, id: data.user.id, property: data.user.property });
      navigate('/dashboard');
    } catch (err) {
        ...
        <div className="w-full max-w-[360px] animate-in slide-in-from-bottom-4 fade-in duration-500 ease-out">
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            <div className="space-y-4 pt-2">
              ...
              <Button type="button" variant="secondary" onClick={handleGoogleSignup} disabled={loading} className="w-full bg-white dark:bg-transparent text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-900">
