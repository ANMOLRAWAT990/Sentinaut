...
const PricingPage = () => {
  ...
  const [firstProperty, setFirstProperty] = React.useState('');
  ...
  const handleUpgrade = async (planTier, amount) => {
    ...
    const targetProp = activeProperty || firstProperty || user.property || 'Unassigned';
    const API_URL = import.meta.env.VITE_API_URL || 'http://127.0.0.1:8000';
      ...
      <main className="flex-grow pt-24 pb-32">
        <div className="px-6 sm:px-8 lg:px-16 max-w-6xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-slate-500 font-light text-lg mb-8">Simple, transparent, and built for scale.</p>
            
            <div className="flex items-center justify-center gap-4 text-sm font-medium">
              <span className={!isYearly ? 'text-slate-900 dark:text-white' : 'text-slate-500'}>Monthly</span>
              <button 
                onClick={() => setIsYearly(!isYearly)} 
                className="w-12 h-6 bg-slate-200 dark:bg-slate-800 rounded-full relative transition-colors focus:outline-none"
              >
                <div className={`w-4 h-4 bg-slate-900 dark:bg-white rounded-full absolute top-1 transition-transform ${isYearly ? 'translate-x-7' : 'translate-x-1'}`} />
              </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border-y md:border-x border-slate-200 dark:border-slate-800 divide-y md:divide-y-0 md:divide-x divide-slate-200 dark:divide-slate-800 bg-white dark:bg-slate-950">
            <div className="p-12 sm:p-16 text-center hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors flex flex-col h-full">
              ...
              <p className="text-5xl font-serif text-slate-900 dark:text-white mb-8">₹{isYearly ? '9,990' : '999'}<span className="text-lg font-light text-slate-500">{isYearly ? '/yr' : '/mo'}</span></p>
            ...
            <div className="p-12 sm:p-16 text-center bg-slate-900 dark:bg-white text-white dark:text-slate-900 relative flex flex-col h-full">
              ...
              <p className="text-5xl font-serif mb-8">₹{isYearly ? '29,990' : '2,999'}<span className="text-lg font-light opacity-70">{isYearly ? '/yr' : '/mo'}</span></p>
